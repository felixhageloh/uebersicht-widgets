...
command: 'curl -s "http://www.history.com/this-day-in-history/rss"'
...
update: (output, domEl) -> 
  ...
  # Spit out the date and the title.
  theDate = new Date(theTitle[0])
  theDate = 'Today in ' + theDate.getFullYear()
  theTitle = theTitle[1]
  ...
  # We grab the innerHTML to avoid the "<P>" tags in the feed.
  if parCount > 0
    dom.find(description).html(dom.find(description).find('p')[0].innerHTML)
